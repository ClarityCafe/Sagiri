...
const BASE_SITE_LIST: SiteList = {
  ...
  */
  '3': {
    ...
    backupURL: data =>
      `http://doujinshi.mugimugi.org/index.php?P=BOOK&ID=${data.data.ddb_id}`,
  ...
  '8': {
    ...
    backupURL: data =>
      `http://seiga.nicovideo.jp/seiga/im${data.data.seiga_id}`,
    getRating: () => RATINGS[0]
  },
  '9': {
    name: 'Danbooru',
    ...
    backupURL: data =>
      `https://danbooru.donmai.us/posts/${data.data.danbooru_id}`,
    getRating: (data: string) =>
      RATINGS[
        [null, 'Safe', 'Questionable', 'Explicit'].indexOf(
          data.match(/<li>Rating: (.*?)<\/li>/i)![1]
        )
      ]
  },
  '10': {
  ...
  '12': {
    ...
    getRating: (body: string) =>
      RATINGS[
        [null, 'Safe', 'Questionable', 'Explicit'].indexOf(
          body.match(/<li>Rating: (.*?)<\/li>/i)![1]
        )
      ]
  },
  '13': {
  ...
  '26': {
    ...
    getRating: (body: string) =>
      RATINGS[
        [null, 'Safe', 'Questionable', 'Explicit'].indexOf(
          body.match(/<li>Rating: (.*?)<\/li>/i)![1].split(' ')[0]
        )
      ]
  },
  '27': {
...
const bits = Object.keys(SITE_LIST).map(v => [
  v,
  parseInt(`1${'0'.repeat(parseInt(v))}`, 2)
]);

export const MASKS: Masks = bits.reduce(
  (prev, [key, bin]) => {
    prev[key] = bin;
    return prev;
  },
