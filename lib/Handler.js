...
class Handler {
     ...
     *
     * @param {String | Buffer} file Either a file or URL that you want to find the source of.
    ...
    getSauce(file) {
        return new Promise((resolve, reject) => {
            if (typeof file !== 'string' || typeof file !== Buffer) return reject(new Error('file is not a string nor buffer.'));
            ...
            sendForm(form).then(res => {
                ...
                let results = res.results.filter(r => Constants.SITE_LIST[r.header.index_name.match(/^Index #(\d+):? /)[1]]);
