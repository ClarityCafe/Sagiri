 ...
 * @prop {Number} dbMask Mask for selecting specific indexes to ENABLE. dbmask=8191 will search all of the first 14 indexes. If intending to search all databases, the db=999 option is more appropriate.
 * @prop {Number} dbMaskI Mask for selecting specific indexes to DISABLE. dbmaski=8191 would search only indexes higher than the first 14. This is ideal when attempting to disable only certain indexes, while allowing future indexes to be included by default.
 * @prop {Boolean} testMode Causes each index which has a match to output at most 1 for testing. Works best with a numres greater than the number of indexes searched.
 * @prop {Ratelimiter} shortLimiter Ratelimiter object that takes care of the short period, usually 30 seconds.
 * @prop {Number} outputType type of output that the API prints out. 1 for XML (not implemented), 2 for JSON result.
...
class Handler {
    ...
    constructor(key, options={}) {
        ...
        this.testMode = options.testMode != null ? options.testMode: 1;
        this.dbMask = options.dbMask != null ? options.dbMask: 999;
        this.dbMaskI = options.dbMaskI || null;
        this.outputType = options.outputType != null ? options.outputType: 2;
        ...
        if(this.dbMask === '1') return new Error('XML Support is not yet added in SauceNAO. Please use JSON instead.');
    }
    ...
    getSauce(file) {
        return new Promise((resolve, reject) => {
            ...
            form.append('output_type', this.outputType);
            form.append('testmode', this.testMode);
            if (this.dbMask && this.dbMaskI !== null) {
                 form.append('dbmask', this.dbMask);
                 form.append('dbmaski', this.dbMaskI);   
            }
